...
exports.COLUMN_ALIASES = exports.TEMPLATE_COLUMNS = void 0;
exports.getAllKeywords = getAllKeywords;
exports.suggestMapping = suggestMapping;
exports.mergeMapping = mergeMapping;
exports.mappingSuggestionsToRecord = mappingSuggestionsToRecord;
...
function getAllKeywords() {
    const all = new Set();
    for (const col of exports.TEMPLATE_COLUMNS) {
        const normalizedColumn = normalizeText(col);
        if (normalizedColumn.length >= 3)
            all.add(normalizedColumn);
        for (const alias of exports.COLUMN_ALIASES[col] || []) {
            const normalizedAlias = normalizeText(alias);
            if (normalizedAlias.length >= 3)
                all.add(normalizedAlias);
        }
    }
...
function buildSuggestion(templateColumn, sourceColumn, confidenceScore, method) {
    return {
        templateColumn,
        sourceColumn,
...
function suggestMapping(sourceHeaders) {
    const usedSources = new Set();
    const results = [];
    for (const templateCol of exports.TEMPLATE_COLUMNS) {
        if (AUTHORITATIVE_TEMPLATE_COLUMNS.has(templateCol)) {
            results.push(buildSuggestion(templateCol, null, 0, "none"));
            continue;
        }
        let best = buildSuggestion(templateCol, null, 0, "none");
        const candidates = [
            normalizeText(templateCol),
            ...(exports.COLUMN_ALIASES[templateCol] || []).map(normalizeText),
        ];
        for (const source of sourceHeaders) {
            ...
            const containsMatch = !isInternalSource &&
                allowsPartialMatch &&
                candidates.some((candidate) => candidate.length >= 3 &&
        ...
        if (best.sourceColumn)
            usedSources.add(best.sourceColumn);
        results.push(best);
    }
    return results;
}
function mergeMapping(autoMapping, manualMapping = {}) {
    return {
        ...autoMapping,
        ...Object.fromEntries(Object.entries(manualMapping).filter(([, sourceColumn]) => sourceColumn !== undefined)),
    };
}
function mappingSuggestionsToRecord(mapping) {
    return Object.fromEntries(mapping.map((item) => [item.templateColumn, item.sourceColumn || ""]));
}
