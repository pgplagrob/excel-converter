...
exports.detectSheetProfile = detectSheetProfile;
...
function rowHasAssetLikeStructure(row) {
    const cells = row.map(cellText);
    return cells.some((cell) => /\d/.test(cell) && /[-/]/.test(cell));
}
function rowHasNumericSequence(row) {
    return row.some((cell) => /^\d+$/.test(normalizeText(cell)));
}
function scoreHeaderProfile(rows, profile, tokens, minimumMatches) {
    let best = {
        profile,
        score: 0,
        rowIndex: null,
    ...
    rows.forEach((row, rowIndex) => {
        ...
        if (matched.length < minimumMatches)
            return;
        const structureBonus = rowHasAssetLikeStructure(row) || rowHasNumericSequence(row) ? 0.05 : 0;
...
function scoreDisposalProfile(rows) {
...
function scoreSummaryProfile(rows, sheetName) {
...
function scoreNameOnlySkipProfile(sheetName, rows) {
    const compactSheetName = normalizeText(sheetName);
...
function scoreRealEstateProfile(rows, sheetName) {
    const score = scoreHeaderProfile(rows, "realEstate", REAL_ESTATE_HEADERS, 2);
    const compactSheetName = normalizeText(sheetName);
    const matchedName = ["อสังหาริมทรัพย์", "ที่ดิน", "อาคาร", "สิ่งปลูกสร้าง"].filter((token) => compactSheetName.includes(normalizeText(token)));
...
function detectSheetProfile(rows, sheetName) {
