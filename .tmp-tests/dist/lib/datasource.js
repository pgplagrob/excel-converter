...
function rowContainsAll(row, tokens) {
...
function sheetLooksAssetLike(matrix) {
    const scanRows = matrix.slice(0, Math.min(matrix.length, 80));
    const hasAssetHeader = scanRows.some((row) => rowContainsAny(row, ["รหัสสินทรัพย์", "รหัสครุภัณฑ์", "AssetCode"]) &&
...
function deriveAssetCategoryFromText(value) {
    if (!value)
        return "";
    if (/อสังหาริมทรัพย์|ที่ดิน|อาคาร|สิ่งปลูกสร้าง/.test(value))
        return "อสังหาริมทรัพย์";
    return "ครุภัณฑ์";
}
function findHeaderRowByTokens(matrix, requiredTokens) {
...
function legacyProfileFromDetection(detection) {
    ...
    if (detection.profile === "assetData")
        return "ASSET_DATA";
...
function parseTransferSheet(sheetName, matrix) {
    ...
    for (let index = headerRowIndex + 1; index < matrix.length; index += 1) {
        ...
        row[exports.INTERNAL.seq] = sourceRow[0] ?? "";
        setNormalizedFields(row, {
...
function parseAssetDataSheet(sheetName, matrix) {
    ...
    for (let index = headerRowIndex + 1; index < matrix.length; index += 1) {
        ...
        const row = withCommonMeta(buildRawRow(headers, sourceRow), "ASSET_DATA", sheetName, index + 1, sourceAssetType, Boolean(sourceAssetType), "", assetName);
        setNormalizedFields(row, {
            assetCode,
            assetName,
            assetDetail: "",
        ...
        row[exports.INTERNAL.status] = cellText(valueAt(sourceRow, statusIndex)) || "ปกติ";
        row[exports.INTERNAL.assetCategory] = deriveAssetCategoryFromText(sourceAssetType);
        row[exports.INTERNAL.needCount] = "True";
        row[exports.INTERNAL.importantFlag] = "False";
        row[exports.INTERNAL.depreciationFlag] = "True";
        rows.push(row);
    ...
    if (!rows.length)
        warnings.push("ไม่พบแถว AssetData ที่มี AssetCode หรือ ModelName");
    return {
        sheetName,
...
function appendDataQualityWarnings(sheet, matrix) {
    // 1) รหัสครุภัณฑ์ซ้ำ
    const codeRows = new Map();
    for (const row of sheet.rows) {
        const code = cellText(row[exports.INTERNAL.assetCode]);
        if (!code)
            continue;
        const excelRow = Number(row[exports.SOURCE_EXCEL_ROW_COLUMN]) || 0;
    ...
    for (const row of matrix) {
        if (!row.some((cell) => /รวมทั้งสิ้น/.test(cellText(cell))))
            continue;
        const stated = parseInt(cellText(row[0]).replace(/[,\s]/g, ""), 10);
        if (!Number.isNaN(stated) && stated !== sheet.rowCount) {
            sheet.warnings.push(`ยอดรวมในไฟล์ (รวมทั้งสิ้น = ${stated}) ไม่ตรงกับจำนวนที่อ่านได้ (${sheet.rowCount}) ต่าง ${Math.abs(stated - sheet.rowCount)} แถว`);
        }
        break;
    }
    // 3) ชื่อสินทรัพย์สั้นผิดปกติ (มักกรอกไม่ครบ)
    const shortNames = sheet.rows
        .map((row) => ({ name: cellText(row[exports.INTERNAL.assetName]), code: cellText(row[exports.INTERNAL.assetCode]) }))
